...
// Renamed prop from selectedTooth to selectedTeeth (plural)
function Model({ selectedTeeth, onTeethLoaded, onMeshClick }) { // Accept selectedTeeth, onTeethLoaded, and onMeshClick props
  ...
  }, [scene]); // Dependency array only includes scene
...
function DisplayWholeMouth() {
  ...
  const handleCanvasClick = (event) => {
    ...
    // This might need further refinement depending on the exact event structure from r3f
    if (event.target === event.currentTarget && event.intersections?.length === 0) {
       console.log("Canvas background clicked. Selection remains unchanged.");
       // No action needed here anymore to clear selection
