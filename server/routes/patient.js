...
router.post('/save-patient', (req, res) => {
    ...
    const { patientName, nhiNumber, dateOfBirth, address, phone, teethLayout } = req.body;
    ...
    fs.writeFile(filePath, updatedPatients, 'utf8', (err) => {
        ...
        res.status(200).json({ message: 'Patient saved successfully!', patientId: newPatientId });
        console.log("success")
    });
});
...
router.post('/update-patient', (req, res) => {
    ...
    fs.writeFile(filePath, updatedPatients, 'utf8', (err) => {
      if (err) {
        ...
        return res.status(500).json({ error: 'Could not update patient information' });
      }
...
router.put('/updateinfo/:nhiNumber', (req, res) => {
  ...
  if (!name || !dateOfBirth || !address || !phone) {
      ...
      return res.status(400).json({ error: 'All fields (name, dateOfBirth, address, phone) are required.' });
  }
  ...
  fs.writeFile(filePath, updatedPatients, 'utf8', (err) => {
      ...
      res.status(200).json({ message: 'Patient information updated successfully!', patient: patients[normalizedNhi] });
      console.log("success");
  });
});
